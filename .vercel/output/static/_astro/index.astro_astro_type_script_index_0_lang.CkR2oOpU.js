window.switchToUnit = function (o) {
	try {
		const n = document.getElementById('temp-toggle-checkbox')
		n && (n.checked = o === 'fahrenheit')
		const a = document.getElementById('currentTemp')
		if (a) {
			const t = parseFloat(a.getAttribute('data-temp-c') || '0')
			if (!isNaN(t))
				if (o === 'celsius') a.textContent = `${t}°C`
				else {
					const r = Math.round((t * 9) / 5 + 32)
					a.textContent = `${r}°F`
				}
		}
		;(document.querySelectorAll('.daily-temp [data-temp-c]').forEach(t => {
			const r = parseFloat(t.getAttribute('data-temp-c') || '0')
			if (!isNaN(r))
				if (o === 'celsius') t.textContent = `${r}°C`
				else {
					const m = Math.round((r * 9) / 5 + 32)
					t.textContent = `${m}°F`
				}
		}),
			localStorage.setItem('temperatureUnit', o))
	} catch (n) {
		console.error('Error switching temperature unit:', n)
	}
}
document.addEventListener('DOMContentLoaded', function () {
	const o = new URLSearchParams(window.location.search),
		n = o.has('lat') && o.has('lon'),
		a = document.querySelector('.weather-container')?.innerHTML.includes('📍')
	!n && !a && window.loadWeather(51.5074, -0.1278, 'London')
	const e = localStorage.getItem('temperatureUnit') || 'celsius'
	window.switchToUnit && window.switchToUnit(e)
	const t = document.getElementById('temp-toggle-checkbox')
	;(t &&
		t.addEventListener('change', function () {
			const u = this.checked ? 'fahrenheit' : 'celsius'
			window.switchToUnit && window.switchToUnit(u)
		}),
		document.querySelectorAll('button[id^="loc-"]').forEach(u => {
			u.addEventListener('click', function (l) {
				const c = l.currentTarget,
					d = c.dataset.lat,
					s = c.dataset.lon,
					i = c.dataset.city
				d && s && i && window.loadWeather(parseFloat(d), parseFloat(s), i)
			})
		}),
		document.querySelectorAll('a[href*="lat="]').forEach(u => {
			u.addEventListener('click', function (l) {
				const c = l.currentTarget
				;((c.style.transform = 'scale(0.95)'), (c.style.opacity = '0.7'))
            ...
            `),
					(d.textContent = '🌍 Loading weather...'),
					document.body.appendChild(d))
			})
		}))
})
window.loadWeather = async function (o, n, a) {
	try {
		...
		e &&
            ...
            `)
		const t = document.getElementById('currentCity')
		t && (t.textContent = `Loading ${a}...`)
		const r = document.getElementById('currentCoords')
		r && (r.textContent = `${o}, ${n}`)
		...
		if (!c || !c.includes('application/json')) {
			...
			throw (
				console.error('Non-JSON response:', i),
				new Error(
					`Server returned ${c || 'unknown content type'} instead of JSON`,
				)
			)
		}
		...
		try {
			...
			throw (
				console.error('JSON parse error:', i),
				console.error('Response text:', d),
				new Error(
					`Invalid JSON response: ${i instanceof Error ? i.message : 'Parse error'}`,
				)
			)
		}
		if (l.ok && s && !s.error) {
			h(s)
			const i = document.getElementById('currentCity')
			i && (i.textContent = s.location?.city || a)
		} else {
			let i = 'Unknown error'
			;(s?.message
				? (i =
				...
				: s?.error
					? (i = typeof s.error == 'string' ? s.error : JSON.stringify(s.error))
					: (i = `HTTP ${l.status} - ${l.statusText}`),
				console.error('API error:', i),
				console.error('Full error response:', s),
				f(`Failed to load weather data: ${i}`))
		}
	} catch (e) {
		;(console.error('Weather load error:', e),
			e instanceof Error
				? e.name === 'AbortError'
					? f('Request timeout - please try again')
					: f(`Network error: ${e.message}`)
				: f(`Unexpected error: ${String(e)}`))
	}
}
window.loadWeatherFromInputs = function () {
	const o = document.getElementById('manualLat'),
		n = document.getElementById('manualLon'),
		a = o?.value,
		e = n?.value
	if (!a || !e) {
	...
	window.loadWeather(parseFloat(a), parseFloat(e), 'Custom Location')
}
function p(o, n = !1) {
	const a = n ? 'text-8xl' : 'text-4xl'
	let e = '☁️',
		t = ''
	switch (o) {
		case 'meteocons:clear-day-fill':
			;((e = '☀️'), (t = 'animate-bounce'))
			break
		case 'meteocons:clear-night-fill':
			;((e = '🌙'), (t = 'animate-pulse'))
			break
		case 'meteocons:partly-cloudy-day-fill':
			;((e = '⛅'), (t = 'animate-partly-cloudy'))
			break
		case 'meteocons:partly-cloudy-night-fill':
			;((e = '🌙'), (t = 'animate-partly-cloudy-slow'))
			break
		...
		case 'meteocons:snow-fill':
			;((e = '🌨️'), (t = 'animate-snow-float'))
			break
		case 'meteocons:sleet-fill':
			;((e = '🌨️'), (t = 'animate-snow-fast'))
			break
		case 'meteocons:hail-fill':
			;((e = '🌨️'), (t = 'animate-hail-bounce'))
			break
		case 'meteocons:fog-fill':
			;((e = '🌫️'), (t = 'animate-fog-swirl'))
			break
		case 'meteocons:wind-fill':
			;((e = '💨'), (t = 'animate-wind-sway'))
			break
		...
		case 'meteocons:wind-strong-fill':
			...
			break
		default:
			o.includes('clear') && o.includes('day')
				? ((e = '☀️'), (t = 'animate-bounce'))
				: o.includes('clear') && o.includes('night')
					? ((e = '🌙'), (t = 'animate-pulse'))
					: o.includes('partly-cloudy') || o.includes('partial')
						? ((e = '⛅'), (t = 'animate-partly-cloudy'))
						: o.includes('rain') || o.includes('drizzle')
							? ((e = '🌧️'), (t = 'animate-rain-drop'))
							: o.includes('thunder') || o.includes('storm')
								? ((e = '⛈️'), (t = 'animate-thunder-pulse'))
								: o.includes('snow')
									? ((e = '🌨️'), (t = 'animate-snow-float'))
									: o.includes('sleet') || o.includes('hail')
										? ((e = '🌨️'), (t = 'animate-hail-bounce'))
										: o.includes('fog') ||
											  o.includes('mist') ||
											  o.includes('haze')
											...
											: o.includes('wind')
												? ((e = '💨'), (t = 'animate-wind-sway'))
												: o.includes('tornado') ||
													  o.includes('hurricane') ||
													  o.includes('extreme')
													? ((e = '🌪️'), (t = 'animate-spin'))
													: o.includes('cloudy') || o.includes('overcast')
														? ((e = '☁️'), (t = 'animate-pulse'))
														: ((e = '☁️'),
															(t = 'animate-pulse'),
															console.warn(
																`Unknown weather icon: ${o}, using default fallback`,
															))
			break
	}
	return `<span class="${a} ${t} inline-block hover:scale-110 transition-all duration-300 cursor-pointer" title="${o}">${e}</span>`
}
function h(o) {
	const n = document.querySelector('.weather-container')
	if (!n) return
	const a = o.daily
		.slice(0, 3)
		.map(
	...
	t &&
		t.addEventListener('change', function () {
			const r = this.checked ? 'fahrenheit' : 'celsius'
			window.switchToUnit && window.switchToUnit(r)
		})
}
function f(o) {
	const n = document.querySelector('.weather-container')
