...
export const GET: APIRoute = async ({ url }) => {
	try {
		...
		// Get city name using reverse geocoding with robust error handling
		...
		try {
			...
			const geocodeTimeoutId = setTimeout(() => geocodeController.abort(), 5000) // 5 second timeout for geocoding
		...
		// Get next 3 days weather forecast with validation (skip today, start from tomorrow)
		const dailyForecast = []
		if (
			data.daily?.time &&
		...
		// Ensure we have at least some forecast data, even if empty
		while (dailyForecast.length < 3) {
			...
			dailyForecast.push({
				...
				maxTemp: Math.round(currentTemp + Math.random() * 5 - 2.5), // Fallback with slight variation
		...
		// Return simplified weather data with safe defaults
		const weatherData = {
			location: {
